...
int main() {
    ...
    int iteration = 0;
    double alpha = 0.1;               // 共轭梯度步长（可根据实际情况调整）
    const double conv_threshold = 1e-6;
    const int max_iter = 1000;

    // 用于保存最后一次的梯度、搜索方向和上一次的梯度
    std::vector<Gradient> finalGrads;
    std::vector<Gradient> searchDir;  // 搜索方向 p
    std::vector<Gradient> prevGrad;   // 上一次的梯度
    ...
    while (iteration < max_iter) {
        ...
        // 解析 Gaussian 输出文件（假定输出文件为 "opt_step{iteration}.log"）
        ...
        // 共轭梯度更新：更新原子坐标，同时更新搜索方向和上一次的梯度
        conjugate_gradient_update(atoms, grads, searchDir, prevGrad, alpha);
